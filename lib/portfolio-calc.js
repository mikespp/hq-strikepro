...
// ── พอร์ต Master (MT5 loop-login snapshots) ─────────────────────────────────────
// rows: pf_daily [{login, d, balance, equity, deposit, withdrawal}] (one row per
// account per day, refreshed hourly). accounts: pf_accounts [{login, label, currency}].
// Returns per-master rolling windows + a daily-return calendar + a sparkline curve,
// plus an AUM-weighted fund aggregate. Returns are TIME-WEIGHTED (deposit-neutral).

const DAY = 86400000;
const ymd = dt => dt.toISOString().slice(0, 10);
const epoch = d => Math.floor(Date.parse(d + 'T00:00:00Z') / 1000);
const shiftDays = (d, n) => ymd(new Date(Date.parse(d + 'T00:00:00Z') + n * DAY));

// Compound the daily returns whose date passes `pred` into a % gain.
function compound(rets, pred) {
  let cf = 1, hit = false;
  for (const x of rets) if (pred(x.d)) { cf *= (1 + x.r); hit = true; }
  return hit ? round((cf - 1) * 100, 2) : 0;
}
function drawdownOf(cumCurve) {
  let peak = 0, maxDD = 0;
  for (const p of cumCurve) { const eq = 1 + p.v / 100; const pk = 1 + peak / 100;
    if (eq > pk) peak = p.v; const dd = (eq / (1 + peak / 100) - 1) * 100; if (dd < maxDD) maxDD = dd; }
  return round(maxDD, 2);
}

function computePortfolio(rows, accounts) {
  const labels = new Map((accounts || []).map(a => [Number(a.login), a.label]));
  const ccy    = new Map((accounts || []).map(a => [Number(a.login), a.currency || 'USD']));

  const byLogin = new Map();
  for (const r of rows) { const k = Number(r.login); if (!byLogin.has(k)) byLogin.set(k, []); byLogin.get(k).push(r); }

  const dayAgg = new Map(); // fund: d -> { wsum, wr }
  const masters = [];

  for (const [login, arr] of byLogin) {
    const sorted = arr.slice().sort((a, b) => (a.d < b.d ? -1 : a.d > b.d ? 1 : 0));
    const last = sorted[sorted.length - 1];
    if (!last) continue;
    const rets = accountReturns(sorted);            // [{ d, r, weight }]
    // A brand-new master has only one snapshot (no prior day) → no returns yet.
    // Still show it: current equity/balance with 0% gains until history builds up.
    const lastD = rets.length ? rets[rets.length - 1].d : last.d;
    const curMonth = lastD.slice(0, 7), curYear = lastD.slice(0, 4);
    const wk = shiftDays(lastD, -6), q = shiftDays(lastD, -89);

    // cumulative sparkline curve (start at 0)
    let cf = 1; const curve = [{ t: epoch(shiftDays(sorted[0].d, 0)), v: 0 }];
    const daily = [];
    for (const x of rets) {
      cf *= (1 + x.r);
      curve.push({ t: epoch(x.d), v: round((cf - 1) * 100, 2) });
      daily.push({ d: x.d, r: round(x.r * 100, 2) });
      const a = dayAgg.get(x.d) || { wsum: 0, wr: 0 };
      a.wsum += x.weight; a.wr += x.weight * x.r; dayAgg.set(x.d, a);
    }
    masters.push({
      login, accountId: String(login), name: labels.get(login) || ('#' + login), currency: ccy.get(login) || 'USD',
      equity: round(last.equity, 2), balance: round(last.balance, 2),
      gainToday: rets.length ? round(rets[rets.length - 1].r * 100, 2) : 0,
      gainWeek:  compound(rets, d => d >= wk),
      gainMonth: compound(rets, d => d.slice(0, 7) === curMonth),
      gain3m:    compound(rets, d => d >= q),
      gainYtd:   compound(rets, d => d.slice(0, 4) === curYear),
      gainAll:   round((cf - 1) * 100, 2),
      maxDrawdown: drawdownOf(curve),
      lastDate: lastD,
  ...
  // fund aggregate (capital-weighted daily returns)
  const days = [...dayAgg.keys()].sort();
  const fundRets = days.map(d => { const a = dayAgg.get(d); return { d, r: a.wsum > 0 ? a.wr / a.wsum : 0 }; });
  let cf = 1; const fundCurve = days.length ? [{ t: epoch(days[0]), v: 0 }] : [];
  for (const x of fundRets) { cf *= (1 + x.r); fundCurve.push({ t: epoch(x.d), v: round((cf - 1) * 100, 2) }); }
  const lastD = days.length ? days[days.length - 1] : '';
  const curMonth = lastD.slice(0, 7), curYear = lastD.slice(0, 4);
  const wk = lastD ? shiftDays(lastD, -6) : '', q = lastD ? shiftDays(lastD, -89) : '';

  masters.sort((a, b) => b.equity - a.equity);

  return {
    fund: {
      masters:     masters.length,
      equity:      round(masters.reduce((s, m) => s + m.equity, 0), 2),
      aum:         round(masters.reduce((s, m) => s + m.equity, 0), 2),
      gainToday:   fundRets.length ? round(fundRets[fundRets.length - 1].r * 100, 2) : 0,
      gainWeek:    compound(fundRets, d => d >= wk),
      gainMonth:   compound(fundRets, d => d.slice(0, 7) === curMonth),
      gain3m:      compound(fundRets, d => d >= q),
      gainYtd:     compound(fundRets, d => d.slice(0, 4) === curYear),
      gainAll:     round((cf - 1) * 100, 2),
      maxDrawdown: drawdownOf(fundCurve),
      lastDate:    lastD || null,
    },
    fundCurve: downsample(fundCurve, 90),
    masters,
...
module.exports = { computeFund, accountReturns, computeMasters, computePortfolio };
