...
const MAIL_FROM = process.env.MAIL_FROM ||
  (GMAIL_USER ? `Bussay Community <${GMAIL_USER}>` : 'Bussay Community <onboarding@resend.dev>');

// Parse "Name <email@x>" (or a bare email) into { name, email }
function parseFrom(from) {
  const m = /^\s*(.*?)\s*<\s*([^>]+)\s*>\s*$/.exec(from);
  if (m) return { name: m[1] || 'Bussay Community', email: m[2] };
  return { name: 'Bussay Community', email: (from || '').trim() };
}
  ...
  <div style="font-family:'IBM Plex Sans Thai',-apple-system,Segoe UI,sans-serif;background:#0d0d0d;padding:32px;color:#f1f1f1">
    <div style="max-width:440px;margin:0 auto;background:#1a1a1a;border:1px solid #2e2e2e;border-radius:14px;overflow:hidden">
      ...
      <div style="padding:28px 26px">
        <div style="font-size:18px;font-weight:800">BUSSAY <span style="color:#E63946">COMMUNITY</span></div>
...
async function sendOtpEmail(to, code) {
  const res = await sendMail(to, `รหัสยืนยัน Bussay Community: ${code}`, otpHtml(code));
...
async function sendDiscordVerifyEmail(to, link, username) {
  return sendMail(to, 'ยืนยันบัญชี Discord กับ Bussay Community', discordVerifyHtml(link, username));
}
