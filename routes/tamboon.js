...
// ทำบุญ Bussay "rounds" live in tamboon_editions (date/time/venue only, no seats).
...
// ── GET /api/tamboon/status  (public) — active round's date/time/venue ─────────
router.get('/status', async (req, res) => {
  try {
    const cfg = await activeCfg();
    if (!cfg) return res.json({ round: null });
    const count = await db.countTamboonRegistrations(cfg.round);
...
// ── GET /api/tamboon/me  (member) — has the user RSVP'd the active round? ───────
router.get('/me', requireAuth, async (req, res) => {
  try {
    ...
    res.json({ registered: await db.hasTamboonRegistration(user.email, cfg.round) });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'เกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง' });
  }
...
// ── POST /api/tamboon/join  (member) — one-click RSVP using the profile ────────
router.post('/join', requireAuth, async (req, res) => {
  try {
    ...
    if (await db.hasTamboonRegistration(user.email, cfg.round))
    ...
    try {
      await db.createTamboonRegistration(data, cfg.round);
    } catch (err) {
...
// ── GET /api/tamboon/registrations  (admin) — active round's list ──────────────
router.get('/registrations', requireAdmin, async (req, res) => {
  try {
    const cfg = await activeCfg();
    res.json(await db.listTamboonRegistrations(cfg ? cfg.round : null));
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'เกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง' });
  }
...
// ── PATCH /api/tamboon/registrations/:id/confirm  (admin) — check-in ───────────
router.patch('/registrations/:id/confirm', requireAdmin, async (req, res) => {
  const id = parseInt(req.params.id, 10);
  if (!id) return res.status(400).json({ error: 'invalid id' });
  const value = req.body.value ? 1 : 0;
  try {
    await db.setTamboonFlag(id, value);   // ทำบุญ has no Discord role — just the check-in flag
...
// ── GET /api/tamboon/admin/state  (admin) ──────────────────────────────────────
...
// ── PATCH /api/tamboon/admin/edition  (admin) — edit the active round ───────────
router.patch('/admin/edition', requireAdmin, async (req, res) => {
  try {
    ...
    const p = parseInput(req.body);
    if (p.error) return res.status(400).json({ error: p.error });
    await db.updateTamboonEdition(cfg.round, { event_start: p.event_start, event_end: p.event_end, venue: p.venue });
    await db.upsertTamboonCalendarEvent(p.dateYMD);
